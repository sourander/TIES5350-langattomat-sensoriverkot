...
Tämä raportti on osa Langattomat sensoriverkot -kurssia. Tehtänantona on valita yksi Kandris et al. (2020) esittämistä luokituksista. Kuusi sovellusalueesta tässä työssä keskitytään erityisesti Flora \& Fauna -alueeseen, jossa sensoriverkkoja hyödynnetään kasvi- ja eläinympäristöjen seurannassa.

Johdannon yhteydessä esitellään kaksi valittua tieteellistä artikkelia, jotka kuvaavat konkreettisia sovelluskohteita tällä alueella. Ensimmäisessä artikkelissa, \emph{An Internet of Things (IoT) Application on Volcano Monitoring} (Awadallah et al., 2019), kuvataan LoRa-teknologiaan perustuvan sensoriverkon käyttöä tulivuoren lämpötilan seurantaan. Toisessa artikkelissa, \emph{TreeVibes: Modern Tools for Global Monitoring of Trees for Borers} (Rigakis et al., 2021), tarkastellaan puiden sisäisten värähtelyjen mittaamista ja analysointia haitallisten hyönteisten, kuten sarvijäärien, havaitsemiseksi. Näiden lisäksi tarkastellaan myös ympäristön monitoroinnin turvallisuuteen liittyviä haasteita artikkelin \emph{WSN Protocols and Security Challenges for Environmental Monitoring Applications: A Survey} (Adu-Manu et al., 2022) pohjalta, joka toimii tukena sovellusartikkeleiden tarkastelulle.

Tämän työn tavoitteena on antaa yleiskuva valittujen sovellusten toimintaperiaatteista sekä niiden hyödyntämistä teknologioista ja tuoda esiin keskeisiä haasteita, joita langattomien sensoriverkkojen käyttöönottoon liittyy ympäristön seurannassa. Työssä käsitellään artikkeleita kurssin luentovideoiden ja luentomateriaalien rajaamien aiheiden kautta.

\subsection*{Generatiivisen tekoälyn käyttö}

Tekoälyä on käytetty tiedonhaun apuna. Suoritin sekä perinteistä Google Scholar että Gemini Deep Research tiedonhakua. Jälkimmäisessä käytetty kehote ja taulukkomuotoon parsimani tulokset löytyvät liitteistä (ks. Liite \ref{app:deep-research} ja Liite \ref{app:deep-research-results}). Tekoälyä on käytetty myös muotoilun apuna esimerkiksi kääntäessä minun omista muistiinpanoita Markdown-muotoisia taulukoita LaTeX-muotoon.
...
%                                                     %
% --------------------------------------------------- %
\chapter{Flora & Fauna sovellukset}
...
\todo Selitä lyhyesti, mitä etsin, miksi, ja miten valitsin kyseiset.

Ensimmäinen valittu: Volcano \cite{volcano}

Toinen valittu: TreeVibes \cite{treevibes}, joka löytyi Deep Research -työn parsimisvaiheessa, ja liitteissä se tunnetaan tunnuksella 9B (ks.Liite \ref{app:deep-research-results}). Tekoäly ehdotus ei sisältänyt TreeVibes-julkaisua, mutta julkaisu tunnusnumeroltaan 9 käytti kyseistä sensoria ja viittasi tähän työhön lähteissään. Alkuperäinen artikkeli osoittautui enemmän tähän tehtävänantoon sopivaksi kuin datan analytiikkaa ja koneoppimista painottava jatkotutkimus.

Kolmas valittu julkaisu on kyselytutkimus, joka käsittelee WSN-protokollien turvallisuutta, Scholarilla löytynyt lähde, joka tunnetaan liitteissä tunnusnumerolla 26(ks. Liite \ref{app:google-scholar-results}): \emph{WSN Protocols and Security Challenges for Environmental Monitoring Applications: A Survey} \cite{security}.

IEEE-artikkeleihin eivät Google Scholarin linkit useimmiten toimineet, mutta pääsin niihin käsiksi Jyväskylän Yliopiston aineistohaun kautta. Aineistoa tunnusnumero 17 (ks.Liite \ref{app:deep-research-results}) en saanut jostain syystä ladattua laisinkaan. Sen sisältö on mielenkiintoinen, mutta pudotin sen pois, koska palautuksessa vaaditaan PDF-formaattia. Vain HTML-kokoteksti oli saatavilla.
...
\todo Kuvaa lyhyesti molempien sovellusten taustat: mitä ongelmaa ne ratkaisevat?

\todo Artikkelissa A keskitytään... 

\todo kun taas artikkelissa B lähestymistapa on...
